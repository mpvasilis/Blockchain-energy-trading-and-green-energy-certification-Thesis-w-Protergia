...
//
// Private issuance and private trading / Private issuance and public trading
//
...
contract PrivateIssuer is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event PrivateCertificationRequestApproved(
    ...
    event CertificateMigratedToPublic(
        uint256 indexed _certificateId,
        address indexed _owner,
    ...
    mapping(uint256 => RequestStateChange)
        private _requestMigrateToPublicStorage;

    // Storage for PrivateTransferRequest
    mapping(uint256 => PrivateTransferRequest)
        private _requestPrivateTransferStorage;
    ...
    function initialize(address _issuer) public initializer {
        require(
            _issuer != address(0),
            "PrivateIssuer::initialize: Cannot use address 0x0 as Issuer address."
        );
	...
	*/
    ...
    function getCertificateCommitment(uint256 certificateId)
        public
        view
        returns (bytes32)
    {
    ...
    function approveCertificationRequestPrivate(
        ...
        uint256 certificateId = issuer.approveCertificationRequest(
            _requestId,
            0
        );
        _updateCommitment(certificateId, 0x0, _commitment);

        Issuer.CertificationRequest memory request = issuer
            .getCertificationRequest(_requestId);

        emit PrivateCertificationRequestApproved(
            request.owner,
            _requestId,
            certificateId
        );
    ...
    function issuePrivate(
        ...
        require(
            _to != address(0x0),
            "PrivateIssuer::issuePrivate: Cannot use address 0x0 as _to address."
        );

        uint256 requestId = issuer.requestCertificationFor(_data, _to);

        return approveCertificationRequestPrivate(requestId, _commitment);
    }
    ...
    function requestPrivateTransfer(
        uint256 _certificateId,
        bytes32 _ownerAddressLeafHash
    ) external {
        PrivateTransferRequest
            storage currentRequest = _requestPrivateTransferStorage[
                _certificateId
            ];
		 ...
		 */
        require(
            currentRequest.owner == address(0x0),
            "PrivateIssuer::requestPrivateTransfer:Only one private transfer can be requested at a time."
        );

        _requestPrivateTransferStorage[
            _certificateId
        ] = PrivateTransferRequest({
            owner: _msgSender(),
    ...
    function approvePrivateTransfer(
        ...
        PrivateTransferRequest
            storage pendingRequest = _requestPrivateTransferStorage[
                _certificateId
            ];

        require(
            pendingRequest.owner != address(0x0),
            "PrivateIssuer::approvePrivateTransfer: Can't approve a non-existing private transfer."
        );
        require(
            validateMerkle(pendingRequest.hash, _commitment, _proof),
            "PrivateIssuer::approvePrivateTransfer: Wrong merkle tree"
        );

        _requestPrivateTransferStorage[
            _certificateId
        ] = PrivateTransferRequest({owner: address(0x0), hash: ""});
    ...
    function requestMigrateToPublic(
        uint256 _certificateId,
        bytes32 _ownerAddressLeafHash
    ) external returns (uint256 _migrationRequestId) {
        return
            _requestMigrateToPublicFor(
                _certificateId,
                _ownerAddressLeafHash,
                _msgSender()
            );
    }
    ...
    function requestMigrateToPublicFor(
        uint256 _certificateId,
        bytes32 _ownerAddressLeafHash,
        address _forAddress
    ) external onlyOwner returns (uint256 _migrationRequestId) {
        return
            _requestMigrateToPublicFor(
                _certificateId,
                _ownerAddressLeafHash,
    ...
    function getPrivateTransferRequest(uint256 _certificateId)
        ...
        view
        onlyOwner
        returns (PrivateTransferRequest memory)
    {
    ...
    function getMigrationRequest(uint256 _requestId)
        ...
        view
        onlyOwner
        returns (RequestStateChange memory)
    {
    ...
    function getMigrationRequestId(uint256 _certificateId)
        ...
        view
        onlyOwner
        returns (uint256 _migrationRequestId)
    {
        bool found = false;

        for (uint256 i = 1; i <= _requestMigrateToPublicNonce; i++) {
            if (
                _requestMigrateToPublicStorage[i].certificateId ==
                _certificateId
            ) {
                found = true;
                return i;
    ...
    function migrateToPublic(
        ...
        RequestStateChange storage request = _requestMigrateToPublicStorage[
            _requestId
        ];

        require(
            !request.approved,
            "PrivateIssuer::migrateToPublic: Request already approved"
        );
        require(
            !_migrations[request.certificateId],
            "PrivateIssuer::migrateToPublic: certificate already migrated"
        );
        require(
            request.hash ==
                keccak256(abi.encodePacked(request.owner, _volume, _salt)),
            "PrivateIssuer::migrateToPublic: Requested hash does not match"
        );
        require(
            validateOwnershipProof(
                ...
                _volume,
                _salt,
                _commitments[request.certificateId],
                _proof
            ),
            "PrivateIssuer::migrateToPublic: Invalid proof"
        );
        ...
        emit CertificateMigratedToPublic(
            request.certificateId,
            request.owner,
            _volume
        );
	...
	*/
    ...
    function validateOwnershipProof(
        ...
        bytes32 leafHash = keccak256(
            abi.encodePacked(_ownerAddress, _volume, _salt)
        );
	...
	*/
    ...
    function _updateCommitment(
        ...
        require(
            _commitments[_id] == _previousCommitment,
            "PrivateIssuer::updateCommitment: previous commitment invalid"
        );
    ...
    function _migrationRequestExists(uint256 _certificateId)
        private
        view
        returns (bool)
    {
    ...
    function _requestMigrateToPublicFor(
        uint256 _certificateId,
        bytes32 _ownerAddressLeafHash,
        address _forAddress
    ) private returns (uint256 _migrationRequestId) {
        bool exists = _migrationRequestExists(_certificateId);
        require(
            !exists,
            "PrivateIssuer::_requestMigrateToPublicFor: migration request for this certificate already exists"
        );
