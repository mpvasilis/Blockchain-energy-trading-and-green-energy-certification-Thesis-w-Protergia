...
//
// Issuer.sol is an implementation of an I-REC compliant issuer which has the ability to hide the volume for the generated certificates.
//
...
contract Issuer is Initializable, OwnableUpgradeable, UUPSUpgradeable {
    ...
    event CertificationRequestedBatch(
        address[] indexed _owners,
        uint256[] indexed _id
    );
    event CertificationRequestApproved(
        address indexed _owner,
        uint256 indexed _id,
        uint256 indexed _certificateId
    );
    event CertificationRequestBatchApproved(
        address[] indexed _owners,
        uint256[] indexed _ids,
        uint256[] indexed _certificateIds
    );
    event CertificationRequestRevoked(
        address indexed _owner,
        uint256 indexed _id
    );
        ...
        initializer
    {
        require(
            _registry != address(0),
            "Issuer::initialize: Cannot use address 0x0 as registry address."
        );
    ...
    function setPrivateIssuer(address _privateIssuer) public onlyOwner {
        require(
            _privateIssuer != address(0),
            "Issuer::setPrivateIssuer: Cannot use address 0x0 as the private issuer address."
        );
        require(
            privateIssuer == address(0),
            "Issuer::setPrivateIssuer: private issuance contract already set."
        );
	...
	*/
    ...
    function getCertificationRequest(uint256 _requestId)
        public
        view
        returns (CertificationRequest memory)
    {
        return _certificationRequests[_requestId];
    }

    function requestCertificationFor(bytes memory _data, address _owner)
        public
        returns (uint256)
    {
    ...
    function requestCertificationForBatch(
        bytes[] memory _data,
    ...
    function requestCertification(bytes calldata _data)
        external
        returns (uint256)
    {
    ...
    function revokeRequest(uint256 _requestId) external {
        CertificationRequest storage request = _certificationRequests[
            _requestId
        ];

        require(
            _msgSender() == request.owner ||
                _msgSender() == OwnableUpgradeable.owner(),
            "Issuer::revokeRequest: Only the request creator can revoke the request."
        );
        require(!request.revoked, "Issuer::revokeRequest: Already revoked");
        require(
            !request.approved,
            "Issuer::revokeRequest: You can't revoke approved requests"
        );
    ...
    function revokeCertificate(uint256 _certificateId) external onlyOwner {
        require(
            !_revokedCertificates[_certificateId],
            "Issuer::revokeCertificate: Already revoked"
        );
    ...
    function approveCertificationRequest(uint256 _requestId, uint256 _value)
        public
        returns (uint256)
    {
        require(
            _msgSender() == owner() || _msgSender() == privateIssuer,
            "Issuer::approveCertificationRequest: caller is not the owner or private issuer contract"
        );
        require(
            _requestNotApprovedOrRevoked(_requestId),
            "Issuer::approveCertificationRequest: request already approved or revoked"
        );
        ...
        emit CertificationRequestApproved(
            request.owner,
            _requestId,
            certificateId
        );
    ...
    function approveCertificationRequestBatch(
        ...
        require(
            _msgSender() == owner() || _msgSender() == privateIssuer,
            "Issuer::approveCertificationRequestBatch: caller is not the owner or private issuer contract"
        );

        for (uint256 i = 0; i < _requestIds.length; i++) {
            require(
                _requestNotApprovedOrRevoked(_requestIds[i]),
                "Issuer::approveCertificationRequestBatch: request already approved or revoked"
            );
        }
        ...
        emit CertificationRequestBatchApproved(
            owners,
            _requestIds,
            certificateIds
        );
    ...
    function issue(
        ...
        return approveCertificationRequest(requestId, _value);
    }
    ...
    function issueBatch(
        ...
        return approveCertificationRequestBatch(requestIds, _values);
    }
    ...
    function isRequestValid(uint256 _requestId) external view returns (bool) {
        require(
            _requestId <= _latestCertificationRequestId,
            "Issuer::isRequestValid: certification request ID out of bounds"
        );

        CertificationRequest memory request = _certificationRequests[
            _requestId
        ];
	...
	*/
    ...
    function _requestNotApprovedOrRevoked(uint256 _requestId)
        internal
        view
        returns (bool)
    {
